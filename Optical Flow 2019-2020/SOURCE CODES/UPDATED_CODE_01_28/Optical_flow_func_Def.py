...
def draw_flow_Areas(img, flow, step=8):
    h, w = img.shape[:2]
...
def get_flow_over_threshold(lines, dimension, thresholdX, thresholdY):
    new_lines = []
    ...
    for (x1, y1), (x2, y2) in lines:    
        if dimension == 'x':
            ...
            if thresholdX >= 0 and thresholdY < 0:
...
def sleepNrecord(seconds,cam,out_original,out_opt_flow):
    i=0.05
    while i <= seconds:
        ret, img = cam.read()
