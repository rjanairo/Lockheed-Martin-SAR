...
def recording_setup_Windows(dir_opt_flow):
    ...
    #out_original = cv.VideoWriter(os.path.join(dir_original, 'original_' + time_stamp + '.avi'), fourcc, fps, frame_size)
    out_opt_flow = cv.VideoWriter(os.path.join(dir_opt_flow, 'opt_flow' + time_stamp + '.mp4'), fourcc, fps, frame_size)  # set file to write processed frames with optical flow
...
out_opt_flow = recording_setup_Windows(dir_opt_flow)
...
while True:
    fr_count += 1
    ret, img = cam.read(1) # reads NEXT frame from camera (because we need both prevoius and next to analyze flow)
